...
const stripeWebhookHandler = async (req, res) => {
    ...
    try {
        ...
        event = stripe_config_1.stripeClient.webhooks.constructEvent(req.body, sig, env_config_1.Env.STRIPE_WEBHOOK_SECRET);
    }
    catch (error) {
...
async function handleCheckoutSessionCompleted(session) {
    ...
    if (!stripeSubscriptionId)
        return;
    const subscription = await stripe_config_1.stripeClient.subscriptions.retrieve(stripeSubscriptionId);
    ...
    console.log(`Checkout completed for user${userId}- Status:${status}Stripe status:${subscription.status}`);
}
...
async function handleInvoicePaymentFailed(invoice) {
    ...
    if (!userId)
        return;
    await subscription_model_1.default.findOneAndUpdate({ userId }, {
        $set: {
            plan: null,
    ...
    if (isPlanSwitch && stripeSubscription.status === 'active') {
        await subscription_model_1.default.findOneAndUpdate({ userId }, {
            $set: {
                ...
                stripeCurrentPeriodStart: new Date(stripeSubscription.items.data[0].current_period_start * 1000),
                stripeCurrentPeriodEnd: new Date(stripeSubscription.items.data[0].current_period_end * 1000),
            },
        }, { upsert: true });
