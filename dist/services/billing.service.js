...
exports.swithToSubscriptionPlanService = exports.manageSubscriptionBillingPortalService = exports.upgradeToProSubscriptionService = exports.getUserSubscriptionStatusService = void 0;
...
const getUserSubscriptionStatusService = async (userId) => {
    ...
    const daysLeft = subscriptionDoc.trialEndsAt
        ? Math.max(0, Math.ceil((subscriptionDoc.trialEndsAt.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)))
        : 0;
    const planData = {
        [subscription_model_1.SubscriptionPlanEnum.MONTHLY]: {
            price: (0, format_currency_1.convertToDollarUnit)(subscription_model_1.SubscriptionPriceEnum.MONTHLY),
            billing: 'month',
            savings: null,
            features: subscription_1.planFeatures[subscription_model_1.SubscriptionPlanEnum.MONTHLY],
        },
        [subscription_model_1.SubscriptionPlanEnum.YEARLY]: {
            price: (0, format_currency_1.convertToDollarUnit)(subscription_model_1.SubscriptionPriceEnum.YEARLY),
            billing: 'year',
            savings: 'Save 17%',
...
const upgradeToProSubscriptionService = async (userId, body) => {
    ...
    const priceld = plan === subscription_model_1.SubscriptionPlanEnum.MONTHLY
...
const manageSubscriptionBillingPortalService = async (userId, callbackUrl) => {
    if (!userId)
        throw new app_error_1.BadRequestException('User ID is missing');
    if (!callbackUrl)
        throw new app_error_1.BadRequestException('Callback URL is required');
    ...
    try {
        ...
        if (!portalSession.url) {
            throw new app_error_1.InternalServerException('Failed to create billing portal session');
        }
        ...
        if (err.type === 'StripeInvalidRequestError' &&
            ...
            throw new app_error_1.InternalServerException('Billing portal is not configured in Stripe dashboard');
        }
...
const swithToSubscriptionPlanService = async (userId, body) => {
    ...
    if (!user || !user.subscriptionId.stripeSubscriptionId) {
        throw new app_error_1.UnauthorizedException('you dont have an active subscription to switch');
    }
    if (user.subscriptionId.plan === newPlan) {
        throw new app_error_1.BadRequestException(`You already on the ${newPlan} plan`);
    }
    const subscription = await stripe_config_1.stripeClient.subscriptions.retrieve(user.subscriptionId.stripeSubscriptionId);
    const priceId = newPlan === subscription_model_1.SubscriptionPlanEnum.YEARLY
        ? env_config_1.Env.STRIPE_YEARLY_PLAN_PRICE_ID
        : env_config_1.Env.STRIPE_MONTHLY_PLAN_PRICE_ID;
    if (!priceId)
