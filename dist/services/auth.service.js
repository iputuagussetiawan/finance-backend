...
const registerService = async body => {
    ...
    try {
        await session.withTransaction(async () => {
            ...
            if (existingUser) throw new app_error_1.UnauthorizedException('User already exists');
...
const loginService = async body => {
    ...
    if (!user) throw new app_error_1.NotFoundException('Email/password not found');
    const isPasswordValid = await user.comparePassword(password);
    if (!isPasswordValid) throw new app_error_1.UnauthorizedException('Invalid email/password');
    const { token, expiresAt } = (0, jwt_1.signJwtToken)({ userId: user.id });
    const reportSetting = await report_setting_model_1.default
        .findOne(
            {
                userId: user.id,
            },
